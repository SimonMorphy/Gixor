...
def get_top_1000_data(offset):
    ...
    query = "SELECT location FROM no_lcation_followings GROUP BY location LIMIT 2000 offset %s"
...
def update_data(location, nation):
    connection = pymysql.connect(host='localhost', user='root', password='1234', database='spider')
    cursor = connection.cursor()
    query = "UPDATE no_lcation_followings SET nation = %s WHERE location = %s"
    try:
